// A tiny, SAFE arithmetic language for fitness expressions. This is the heart of
// the sandbox's lesson — "evolution optimizes what you reward, not what you
// meant" — so users must be able to write their own scoring formula. It must NOT
// be JavaScript: no eval, no Function constructor, no property access, no
// globals. We tokenize, parse into a small AST, and evaluate that AST against a
...
// ---------------------------------------------------------------- parser
// Recursive descent with the usual precedence: + - bind looser than * /, which
// bind looser than unary minus and calls. Depth is capped as we recurse.
function parse(src: string): FitnessNode {
  ...
  function parseFactor(depth: number): FitnessNode {
    ...
    if (t.kind === "op" && t.value === "+") {
      next(); // allow a leading unary plus, no-op
...
// Parse into an AST. Throws FitnessSyntaxError on any problem.
...
// Non-throwing validation for live editor feedback.
...
// Evaluate to a final score, sanitized for selection. Note: unlike the design
// doc's "negative -> 0", we KEEP negatives — a flat floor at 0 makes gen-0
// selection random (every hopeless early design ties). We only guard the truly
// broken values: NaN sorts last, infinities are capped.
...
// Break the score into the additive terms a human reads at the top level, e.g.
// "distance * 10 - energyUsed * 0.05" -> [{distance * 10, +900}, {energyUsed *
// 0.05, -45}]. Non-additive expressions collapse to a single term.
...
// Pretty-print a node back to source (for breakdown labels).
