// Body-agnostic SVG viewport. It knows nothing about rovers or crawlers — it
// draws whatever parts a phenotype reports (boxes as rotated polygons, circles
// as circles with a spoke so spin is visible), plus terrain, distance markers, a
// start line, and ghost/trail paths. The camera follows the leader. Graphics are
// pooled and reconciled per frame; because every phenotype in a generation is the
...
const VW = 940;
const VH = 440;
const PPM = 26; // px per metre (bodies are ~1–3 m)
...
export function createSandboxView(): SandboxView {
  const root = svg("svg", {
    viewBox: `0 0 ${VW} ${VH}`,
    class: "es-scene",
    ...
    role: "img",
    "aria-label":
      "Physics viewport: a population of evolving machines on terrain",
  });
  ...
  // Reconcile a phenotype's child elements to its parts array. Recreates children
  // only when the kind sequence changes (i.e. on a template switch).
  ...
  function drawPart(pg: PartGfx, part: RenderPart) {
    if (part.kind === "box" && pg.kind === "box") {
      ...
      // spoke from centre to rim, rotated by the wheel angle, so spin is visible
  ...
  function drawTerrain() {
    ...
    // ever storing a point array. A ~0.5 m step keeps hills smooth on screen.
  ...
  function render(params: {
    ...
    // draw the leader last so it sits above the crowd
